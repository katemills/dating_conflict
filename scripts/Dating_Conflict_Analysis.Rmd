...
packages <- c( "tidyverse", "RODBC", "data.table",
               "ggplot2","lavaan","semPlot", "mice",
               "sjPlot","stargazer","apaTables")
...
demotable<-read.csv("../../../../Data/WebSurvey/Demographics/Demographics.csv")
```
...
puberty<-left_join(puberty,
                   (demotable %>% mutate(SID=sid) %>% select(SID,age8thmm))) %>%
  filter(WID==1,
         age8thmm<180)

puberty<-puberty %>% mutate(Puberty_3group=ifelse(Gender==0,
                             ifelse(PDS_Female<=
                                      quantile(PDS_Female,.25,na.rm=TRUE),"less",
                                    ifelse(PDS_Female>quantile(PDS_Female,.25,na.rm=TRUE) &
                                             PDS_Female<quantile(PDS_Female,.75,na.rm=TRUE),"avg",
                                           ifelse(PDS_Female>=
                                                    quantile(PDS_Female,.75,na.rm=TRUE),"more",NA))),
                             ifelse(Gender==1,
                                    ifelse(PDS_Male<=
                                             quantile(PDS_Male,.25,na.rm=TRUE),"less",
                                           ifelse(PDS_Male>quantile(PDS_Male,.25,na.rm=TRUE) &
                                                    PDS_Male<quantile(PDS_Male,.75,na.rm=TRUE),"avg",
                                                  ifelse(PDS_Male>=
                                                      quantile(PDS_Male,.75,na.rm=TRUE),"more",NA))),NA)))
...
thedata1<-left_join((EMA_count %>%
                   ...
                   (puberty %>%
                      filter(WID==c(1)) %>%
                      select(SID,Gender,Latino,Puberty_3group,age8thmm)),
                    by = c("SID")) 
...
thedata<-thedata[c("SID","Gender","Ethnicity","Puberty_3group",
                   "OppMonitorMean","oldest",
                   "EMAaloneBFGF","DatingConflictMean","threatening_mean",
                   "verbal_mean","WillingRiskySexMean","IntentionRiskSexTotal",
                   "age8thmm")] %>%
  filter(!is.na(Gender)) 
thedata$Puberty_3group<-factor(thedata$Puberty_3group,levels = c("less", "avg", "more"))

```

Table 1 (descriptives)
```{R}
stargazer(as.data.frame(dummydata[c("OppMonitorMean",
                                    "oldest",
                                    "EMAaloneBFGF",
                                    "DatingConflictMean",
                                    "threatening_mean",
          ...
          type = "html",
          out="/PeerInfluence/Papers/Dating Conflict/Tables/Table1all.doc")

dummydata_female<-filter(dummydata, Gender==0)
stargazer(as.data.frame(dummydata_female[c("OppMonitorMean",
                                    "oldest",
                                    "EMAaloneBFGF",
                                    "DatingConflictMean",
                                    "threatening_mean",
          ...
          title="Descriptive Statistics",
          type = "html",
          out="/PeerInfluence/Papers/Dating Conflict/Tables/Table1female.doc")

dummydata_male<-filter(dummydata, Gender==1)
stargazer(as.data.frame(dummydata_male[c("OppMonitorMean",
                                    "oldest",
                                    "EMAaloneBFGF",
                                    "DatingConflictMean",
                                    "threatening_mean",
          ...
          title="Descriptive Statistics",
          type = "html",
          out="/PeerInfluence/Papers/Dating Conflict/Tables/Table1male.doc")

```

Table 2 (correlation table)
```{R}
# Dummy code gender, puberty, and ethnicity for correlation table

dummydata<-thedata %>%
  mutate(Gender=ifelse(Gender=="Female",0,
                       ifelse(Gender=="Male",1,NA)),
         Puberty_3group=ifelse(Puberty_3group=="less",0,
                               ifelse(Puberty_3group=="avg",1,
                                      ifelse(Puberty_3group=="more",2,NA))),
         Ethnicity=as.numeric(as.character(Ethnicity)))

colnames(dummydata)

sjt.corr(dummydata[,c("Ethnicity",
                 "Gender",
                 "Puberty_3group",
                 "OppMonitorMean",
                 "oldest",
         ...
         var.labels = c("Ethnicity",
                 "Gender",
                 "Pubertal Development",
                 "Opportunities Monitoring",
                 "Oldest ever romantic partner",
                 "EMA",
                 "CADRI Score"),
         title ="Bivariate Correlation Between Main Variables",
         triangle = "lower",
         fade.ns = F,
         digits = 2,
         file="/PeerInfluence/Papers/Dating Conflict/Tables/Table2.doc")

```

Tables 3-7
```{r}
## Opportunities Monitoring multiple regression table
OppMon_GEP<-lm(OppMonitorMean ~ age8thmm+ Gender+Ethnicity+Puberty_3group, data=thedata, na.action=na.omit)
OppMon_GEPint<-lm(OppMonitorMean ~ age8thmm+ Gender*Puberty_3group+Ethnicity, data=thedata, na.action=na.omit)
sjt.lm(OppMon_GEP,
       OppMon_GEPint,
       pred.labels = c("Age at baseline",
                       "Gender (Male)",
                       "Ethnicity (Latino)",
                       "Less Developed",
                       "More Developed",
                       "Gender x Less Developed",
                       "Gender x More Developed"),
       show.ci = F,
       show.se = T,
       depvar.labels = c("Model 1",
                         "Model 2"),
       file="/PeerInfluence/Papers/Dating Conflict/Tables/OppMonTable.doc")

## Age oldest romantic partner multiple regression table
Oldest_GEP<-lm(oldest ~ age8thmm+Gender+Ethnicity+Puberty_3group, data=thedata, na.action=na.omit)
Oldest_GEPint<-lm(oldest ~ age8thmm+Gender*Ethnicity+Puberty_3group, data=thedata, na.action=na.omit)
...
sjt.lm(Oldest_GEP,
       ...
       depvar.labels = c("Model 1",
                         "Model 2"),
       file="/PeerInfluence/Papers/Dating Conflict/Tables/AgeOldestTable.doc")

## EMA multiple regression table
EMA_GEP<-lm(EMAaloneBFGF ~ age8thmm+Gender+Ethnicity+Puberty_3group, data=thedata, na.action=na.omit)
EMA_GEPOp<-lm(EMAaloneBFGF ~ age8thmm+Gender+Ethnicity+Puberty_3group+OppMonitorMean, data=thedata, na.action=na.omit)
EMA_GEPOpOl<-lm(EMAaloneBFGF ~ age8thmm+Gender+Ethnicity+Puberty_3group+OppMonitorMean+oldest, data=thedata, na.action=na.omit)
...
sjt.lm(EMA_GEP,
       ...
       pred.labels = c("Age at baseline",
                       ...
                       "More Developed",
                       "Opportunities Monitoring",
                       "Age oldest romantic partner"),
       show.ci = F,
       show.se = T,
       depvar.labels = c("Model 1",
                         "Model 2",
                         "Model 3"),
       file="/PeerInfluence/Papers/Dating Conflict/Tables/EMATable.doc")

## CADRI multiple regression table
CADRI_GEP<-lm(DatingConflictMean ~ age8thmm+Gender+Ethnicity+Puberty_3group, data=thedata, na.action=na.omit)
CADRI_GEPOp<-lm(DatingConflictMean ~ age8thmm+Gender+Ethnicity+Puberty_3group+OppMonitorMean, data=thedata, na.action=na.omit)
CADRI_GEPOpOl<-lm(DatingConflictMean ~ age8thmm+Gender+Ethnicity+Puberty_3group+OppMonitorMean+oldest, data=thedata, na.action=na.omit)
CADRI_GEPOpOlEMA<-lm(DatingConflictMean~ age8thmm+Gender+Ethnicity+Puberty_3group+OppMonitorMean+oldest+EMAaloneBFGF, data=thedata, na.action=na.omit)
...
sjt.lm(CADRI_GEP,
       ...
       depvar.labels = c("Model 1",
                         ...
                         "Model 3",
                         "Model 4"),
       file="/PeerInfluence/Papers/Dating Conflict/Tables/CADRITable.doc")

## CADRI multiple regression interaction table
CADRI_MAINS<-lm(DatingConflictMean~ age8thmm+Gender+Puberty_3group+OppMonitorMean, data=thedata, na.action=na.omit)
CADRI_INT<-lm(DatingConflictMean~ age8thmm+Gender*Puberty_3group*OppMonitorMean, data=thedata, na.action=na.omit)
sjt.lm(CADRI_MAINS,
       CADRI_INT,
       pred.labels = c("Age at baseline",
                       "Gender (Male)",
                       ...
                       "Gender x Less Developed",
                       "Gender x More Developed",
                       "Gender x Opportunities Monitoring",
                       "Opportunities Monitoring x Less Developed",
                       "Opportunities Monitoring x More Developed",
                       "Gender x Less Developed x Opportunities Monitoring",
                       "Gender x More Developed x Opportunities Monitoring"),
       show.ci = F,
       show.se = T,
       depvar.labels = c("CADRI","CADRI"),
       file="/PeerInfluence/Papers/Dating Conflict/Tables/CADRI_int_Table.doc")
...
geom_point(aes(colour=Gender),size=1.5,alpha=.3)+
  geom_smooth(method=lm,
                aes(x=OppMonitorMean, y=DatingConflictMean, colour=Gender, fill=Gender),
                size=.7,
                alpha=0.4)+
scale_fill_manual(values=colour_labels,
                      labels=c('Female', 'Male'))+
scale_colour_manual(values=colour_labels,
                      labels=c('Female', 'Male'))+
  labs(x="Opportunities Monitoring mean score",
...
# Interaction between pubertal, oppmon, gender ~ CADRI

puberty_labels=c("#0058AA","#05233F")
oppmon_gender_puberty_conflict_graph<-ggplot(thedata %>% 
                                       filter(!is.na(DatingConflictMean)) %>% 
                                       filter(!Puberty_3group=="less"),
                                             aes(x=OppMonitorMean, y=DatingConflictMean, 
                                                 colour=Puberty_3group,fill=Puberty_3group)) +
geom_point(aes(colour=Puberty_3group),size=1.5,alpha=.3)+
  geom_smooth(method=lm,
                aes(x=OppMonitorMean, y=DatingConflictMean, colour=Puberty_3group, fill=Puberty_3group),
                size=.8,
                alpha=0.5)+
scale_fill_manual(values=puberty_labels,
                      labels=c('Average', 'More'),
                  guide = FALSE)+
scale_colour_manual(values=puberty_labels,
                      labels=c('Average', 'More'))+
  facet_grid(.~Gender,
             scales='free')+
  labs(x="Opportunities Monitoring mean score",
       y="CADRI mean score",colour="Pubertal \nDevelopment")+
  theme_kate() + theme(legend.position="right")
oppmon_gender_puberty_conflict_graph
...
model.2<-lm(DatingConflictMean~ age8thmm +
             Gender*
             Ethnicity*
             Puberty_2group*
             OppMonitorMean*
...
par(mfrow = c(2, 2))
plot(model.2)
PRESS(linear.model = model.2)
pred_r_squared(linear.model = model.2)
model_fit_stats(model.2)

# Trying to whittle down the model
model.3<-lm(DatingConflictMean~age8thmm+
              Ethnicity*
              Gender*
              Puberty_2group*
              OppMonitorMean*
...
summary(model.3)
anova(model.3)
model_fit_stats(model.3)

# Whittling (seems best model)
model.4<-lm(DatingConflictMean ~ 
              Ethnicity*
              Puberty_2group*
              OppMonitorMean, data=thedata, na.action=na.exclude)
summary(model.4)
anova(model.4)
model_fit_stats(model.4)

```

Differences in ever dated anyone between avg. vs. more pubertally developed girls?
```{r}

  output<-left_join((thedata %>%
             filter(Gender=="Female") %>%
             select(SID, Gender,Puberty_3group)),
          (surveydata %>%
             select(SID,EverDate,WID)),
          by="SID") %>%
  filter(WID==c(5,9,11)) %>%
  group_by(Puberty_3group,EverDate) %>%
  summarize(count=n())
