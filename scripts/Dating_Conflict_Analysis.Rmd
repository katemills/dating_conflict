...
puberty<-left_join(demographics,(surveydata %>%
  ...
  mutate(Puberty_3group=ifelse(Gender==0,
                               ifelse(PDS_Female<=3,1,
                                      ifelse(PDS_Female>3 & PDS_Female<3.8,2,
                                             ifelse(PDS_Female>=3.8,3,NA))),
                               ifelse(Gender==1,
                                      ifelse(PDS_Female<=2.4,1,
                                             ifelse(PDS_Female>2.4 & PDS_Female<3.2,2,
                                                    ifelse( PDS_Female>=3.2,3,NA))),NA)))
...
thedata<-left_join((opportunities %>%
                   ...
                   (puberty %>%
                      filter(WID==c(1)) %>%
                      select(SID,Puberty_Category,PDS_Female,PDS_Male,Gender,Ethnicity,Puberty_3group)),
                    by = c("SID")) %>%
