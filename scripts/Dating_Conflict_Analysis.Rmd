...
thedata<-filter(thedata, Gender=="Female")
####
length(na.omit(thedata$OppMonitorMean))
mean(thedata$OppMonitorMean,na.rm=TRUE)
sd(thedata$OppMonitorMean,na.rm=TRUE)
median(thedata$OppMonitorMean,na.rm=TRUE)
range(thedata$OppMonitorMean,na.rm=TRUE)

length(na.omit(thedata$EMAaloneBFGF))
mean(thedata$EMAaloneBFGF,na.rm=TRUE)
sd(thedata$EMAaloneBFGF,na.rm=TRUE)
median(thedata$EMAaloneBFGF,na.rm=TRUE)
range(thedata$EMAaloneBFGF,na.rm=TRUE)

length(na.omit(thedata$DatingConflictMean))
mean(thedata$DatingConflictMean,na.rm=TRUE)
sd(thedata$DatingConflictMean,na.rm=TRUE)
median(thedata$DatingConflictMean,na.rm=TRUE)
range(thedata$DatingConflictMean,na.rm=TRUE)
...
parent_survey_w1 <-read.csv("/PeerInfluence/Data&Analysis/Data/Qualtrics/PQ/2014/ParentQT1C1.csv",header=TRUE,stringsAsFactors = FALSE) %>%
  filter(!grepl("_2",pid)) %>%
  mutate(parentmonitor = round(rowMeans(cbind(as.numeric(p9ms_01),
                                       as.numeric(p9ms_02),
                                       as.numeric(p9ms_03),
                                       as.numeric(p9ms_04),
                                       as.numeric(p9ms_05),
                                       as.numeric(p9ms_06),
                                       as.numeric(p9ms_07)),na.rm = TRUE),3)) %>%
  select(parentmonitor,pid)%>% 
  mutate(WID=1,
         SID=substring(pid,0,nchar(pid)-2))

parent_survey_w5 <-read.csv("/PeerInfluence/Data&Analysis/Data/Qualtrics/PQ/2015/ParentQ2015.csv",header=TRUE,stringsAsFactors = FALSE)  %>%
  filter(!grepl("_2",pid))%>%
  mutate(parentmonitor = round(rowMeans(cbind(as.numeric(p9ms_01),
                                       as.numeric(p9ms_02),
                                       as.numeric(p9ms_03),
...
parent_survey_w9 <-read.csv("/PeerInfluence/Data&Analysis/Data/Qualtrics/PQ/2016/TAP_Parent_Q_2016.csv",header=TRUE,stringsAsFactors = FALSE) %>%
  filter(!grepl("_2",pid)) %>%
  mutate(parentmonitor = round(rowMeans(cbind(as.numeric(p9ms_01),
                                       as.numeric(p9ms_02),
                                       as.numeric(p9ms_03),
...
parent_survey<-bind_rows(parent_survey_w1,parent_survey_w5,parent_survey_w9) %>%
  filter(!is.na(parentmonitor))%>%
  mutate(SID=as.factor(SID))

student_survey<-surveydata %>%
  filter(WID==c(1,5,9)) %>%
  mutate(studentmonitor = round(rowMeans(cbind(ParHang, ParWhere, ParAfter, ParAway, ParPlans, ParInterests, Par2Days),na.rm = TRUE),3)) %>% 
   select(studentmonitor,SID,WID) %>%
  mutate(SID=as.factor(SID))

surveys<-left_join(parent_survey,student_survey,by=c("SID","WID")) %>%
  filter(!is.na(studentmonitor))


ggplot(surveys, aes(x=parentmonitor,y=studentmonitor,
                    colour=SID)) +
  geom_point()+
  geom_smooth(method=lm,
                aes(x=parentmonitor,y=studentmonitor),
                size=.7,
                alpha=0.2,
                colour="dodgerblue",
                fill = "dodgerblue")+
